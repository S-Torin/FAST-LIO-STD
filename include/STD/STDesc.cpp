...
void read_parameters(ros::NodeHandle& nh, ConfigSetting& config_setting) {
  // pre-process
  nh.param<double>("std/ds_size", config_setting.ds_size_, 0.1);
  nh.param<int>("std/maximum_corner_num", config_setting.maximum_corner_num_, 100);

  // key points
  nh.param<double>("std/plane_merge_normal_thre", config_setting.plane_merge_normal_thre_, 0.1);
  nh.param<double>("std/plane_detection_thre", config_setting.plane_detection_thre_, 0.01);
  nh.param<double>("std/voxel_size", config_setting.voxel_size_, 2.0);
  nh.param<int>("std/voxel_init_num", config_setting.voxel_init_num_, 10);
  nh.param<double>("std/proj_image_resolution", config_setting.proj_image_resolution_, 0.5);
  nh.param<double>("std/proj_dis_min", config_setting.proj_dis_min_, 0.0);
  nh.param<double>("std/proj_dis_max", config_setting.proj_dis_max_, 5.0);
  nh.param<double>("std/corner_thre", config_setting.corner_thre_, 10.0);

  // std descriptor
  nh.param<int>("std/descriptor_near_num", config_setting.descriptor_near_num_, 20);
  nh.param<double>("std/descriptor_min_len", config_setting.descriptor_min_len_, 2.0);
  ...
  nh.param<int>("loop/candidate_num", config_setting.candidate_num_, 50);
  nh.param<int>("loop/sub_frame_num", config_setting.sub_frame_num_, 10);
  nh.param<double>("loop/rough_dis_threshold", config_setting.rough_dis_threshold_, 0.01);
  nh.param<double>("loop/vertex_diff_threshold", config_setting.vertex_diff_threshold_, 0.5);
  nh.param<double>("loop/icp_threshold", config_setting.icp_threshold_, 0.4);
  nh.param<double>("loop/normal_threshold", config_setting.normal_threshold_, 0.2);
  nh.param<double>("loop/dis_threshold", config_setting.dis_threshold_, 0.5);
...
void STDescManager::PlaneGeomrtricIcp(
  ...
  Eigen::Vector3d t = transform.first;
  double para_q[4] = {q.w(), q.x(), q.y(), q.z()};
  double para_t[3] = {t(0), t(1), t(2)};
  problem.AddParameterBlock(para_q, 4, new ceres::QuaternionParameterization());
  ...
  Eigen::Quaterniond q_opt(para_q[0], para_q[1], para_q[2], para_q[3]);
